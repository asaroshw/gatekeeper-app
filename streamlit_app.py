...
def fetch_stock_data(ticker_symbol):
    ...
    # Safely try NSE first, then fallback to BSE
    if not ticker_upper.endswith(('.NS', '.BO')):
        try:
            ...
            try:
                ...
                raise ValueError("Ticker not found on NSE or BSE. Please check your spelling.")
    else:
        stock = yf.Ticker(ticker_upper)
        info = stock.info
        if 'currentPrice' not in info:
            raise ValueError("Ticker not found. Please check your spelling.")
    ...
    try:
        ...
        if fin is not None and not fin.empty and 'Net Income' in fin.index:
            ...
            if len(ni_data) >= 2:
                latest_ni = ni_data.iloc[0]
                oldest_ni = ni_data.iloc[-1]
                metrics['net_income_trend'] = f"Net income moved from INR {oldest_ni:,.0f} to INR {latest_ni:,.0f} over the last {len(ni_data)} years."
        ...
        if bs is not None and not bs.empty and 'Total Debt' in bs.index:
            ...
            if len(td_data) >= 2:
                latest_td = td_data.iloc[0]
                oldest_td = td_data.iloc[-1]
                metrics['debt_trend'] = f"Total Debt moved from INR {oldest_td:,.0f} to INR {latest_td:,.0f} over the last {len(td_data)} years."
...
def generate_report_content(ticker, metrics):
    ...
    user_prompt = f"Analyze this stock: {metrics['name']} ({ticker}). Price: INR {metrics['price']}. P/E: {metrics['pe_ratio']}. Debt/Equity: {metrics['debt_to_equity']}. Margin: {metrics['net_margin']}. Trends: {metrics['net_income_trend']}, {metrics['debt_trend']}."
...
def build_pdf_report(pdf_buffer, ticker, metrics, ai_text):
    ...
    for line in ai_text.split('\n'):
        l_str = line.strip()
        if l_str.startswith("DYNAMIC_SECTOR:"): sector_val = l_str.replace("DYNAMIC_SECTOR:", "").strip()
        elif l_str.startswith("DYNAMIC_DURATION:"): duration_val = l_str.replace("DYNAMIC_DURATION:", "").strip()
        elif l_str.startswith("DYNAMIC_RATING:"): rating_val = l_str.replace("DYNAMIC_RATING:", "").strip()
        elif l_str: clean_lines.append(l_str)
                
    story = [Paragraph("Smart Gains", title_style), Paragraph("Automated Equity Research Report — Institutional Series", subtitle_style), Spacer(1, 15)]
    
    grid_rating_display = f"<font color='{rating_colors.get(rating_val.upper().strip(), '#FFFFFF')}'><b>{rating_val}</b></font>"
    ...
    t.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), colors.HexColor('#2B6CB0')), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#4299E1'))]))
    ...
    for line in clean_lines:
        if any(h in line for h in ["COMPANY OVERVIEW", "FUNDAMENTAL & MOMENTUM ANALYSIS", "MACRO AND SECTOR CATALYSTS", "KEY RISKS", "ACTIONABLE VERDICT"]):
            ...
            story.append(Paragraph(processed_line, body_style)); story.append(Spacer(1, 4))
